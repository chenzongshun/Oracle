--创建用户
create user stu_user17 identified by stu123;

--分配连接数据库的权限
grant create session to stu_user17;

--分配创建表的权限
grant create table to stu_user17;

--分配无限空间
alter user stu_user17 quota unlimited on users;
...
empid number(4),
primary key(empid),
constraint fka9fde35c8a43173d foreign key(empid) references employee(empid),
constraint fka9fde35c3f9c031e foreign key(empid) references department(deptid));
...
insert into attendanceinfo(attinfoid,absentdays,affairleavedays,attdate,holidayotdays,lateminutes,leaveearlyminutes,memo,normalotdays,ondays,sickleavedays,empid)
values(1,4,5,to_char(sysdate,'yyyy-mm'),0,0,0,'全勤',0,21,0,4);
insert into attendanceinfo(attinfoid,absentdays,affairleavedays,attdate,holidayotdays,lateminutes,leaveearlyminutes,memo,normalotdays,ondays,sickleavedays,empid)
values(2,0,0,to_char(sysdate,'yyyy-mm'),0,0,0,'全勤',0,21,0,3);
insert into attendanceInfo (attinfoid,absentdays,affairleavedays,attdate,holidayotdays,lateminutes,leaveearlyminutes,memo,normalotdays,ondays,sickleavedays,empid) 
values(3,0,0,to_char(sysdate,'yyyy-mm'),0,0,0,'出勤状况不太乐观',0,21,0,4);
...
insert into staffchangeinfo (id,empid,ename,newdname,newjobname,olddname,oldjobname,status,transdate)
values(1,2,'李梅','总裁办','总裁助理','投资管理部','秘书','在职',to_date('20110103000000','yyyymmddhh24miss'));
...
select  e.empid,ename,d.deptid,d.dname  from employee e,department d where e.deptid=d.deptid;

--2、查询雇员的考勤信息
select e.empid,ename,a.* from employee e,attendanceInfo a where e.empid=a.empid;

--3、查询雇员绩效信息
select e.empid,e.ename,ee.* from employee e ,evaluation ee where e.empid=ee.empid;
